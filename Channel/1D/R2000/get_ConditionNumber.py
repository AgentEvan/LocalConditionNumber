...
def Boundary_Scalar_PHill(value=0):
    bound_text = "\nboundaryField\n"
    bound_text += "{\n"
    bound_text += "INLET\n"
    bound_text += "{\n"
...
def Boundary_Scalar_Xiao(value=0):
    bound_text = "\nboundaryField\n"
    bound_text += "{\n"
    bound_text += "inlet\n"
    bound_text += "{\n"
...
def writeFoamScalar(varName, data, case='Channel2D'):
    ...
    if (case=='Channel2D'):
        ...
        f.write(Boundary_Scalar_PHill())    
    f.close()


if (__name__ == '__main__'):
    # print('\nPreparation...\n')
    # os.system('get_nutL >log_nutL')
    # os.system('get_FullMatrix >log_mat')
    # os.system('get_LatestTime >log_ltime')
    # os.system('get_PointLocation >log_ptl')
    # os.system('get_Volume >log_vol')
    # os.system('get_divRS_DNS >log_divRS')
    ...
    Kj2 = np.zeros(N)
    for i in range(N):
        Kj1[i] = np.linalg.norm(Ainv1[i,:])*np.linalg.norm(divRS[:,0])/(Uinf+1e-12)
        Kj2[i] = np.linalg.norm(Ainv2[i,:])*np.linalg.norm(divRS[:,0])/(Uinf+1e-12)
